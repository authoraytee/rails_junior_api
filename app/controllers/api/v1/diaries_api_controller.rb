...
module Api
  module V1
    # Main class for Dictionaries API
    class DiariesApiController < ApplicationController
      def index
      ...
      def create
        ...
        if diary.save
          ...
          render json: { status: 'ERROR', message: 'Diary not saved', data: diary.errors },
      ...
      def update
        ...
        if diary.update_attributes(diary_params)
          ...
          render json: { status: 'ERROR', message: 'Diary not updated', data: diary.errors },
                 status: :unprocessable_entity
        end
      end
